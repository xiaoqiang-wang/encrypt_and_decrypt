        ...
        if (in_file.read(in_buffer, valid_file_size))
                {
                        ...
                        uint32_t single_file_byte_count = (total_count/mid_file_count);

                        cout<<"file_total_byte_count  = "<< valid_file_size <<endl;
                        cout<<"single_file_byte_count = "<<single_file_byte_count<<endl;

                        //encode to char
                        char *in_ptr = ((char *)in_buffer);


                        uint32_t write_total_byte = 0;


                        std::string file_name_prefix = "mid";
                        for(uint32_t file_index =0; file_index<=mid_file_count; file_index++){
                                ...
                                cout<<"current_file_index: " << std::dec << file_index << std::endl;


                                char *in_ptr = ((((char *)in_buffer)) + (file_index*single_file_byte_count));

                                uint32_t line_length = 1;
                                for(uint32_t byte_index=0; byte_index<single_file_byte_count; byte_index++){
                                        ...
                                        if((write_total_byte == valid_file_size)){
                                                break;
                                        }
                                }
                ...
                case TEST:
                        {
                                ...
                                std::string max_file_index_str =argv[3];
                                std::string OUT_FILE_NAME =argv[4];

                                uint32_t max_file_index_arg = stoi(max_file_index_str.data());
                                uint32_t valid_file_size = Trans(IN_FILE_NAME, max_file_index_arg);

                                Re_Trans(OUT_FILE_NAME, true, valid_file_size, max_file_index_arg);
                        }
                        break;
                case EN_CODE:
                        {
                                ...
                                std::string max_file_index_str =argv[3];
                                uint32_t max_file_index_arg = stoi(max_file_index_str.data());
                                Trans(IN_FILE_NAME, max_file_index_arg);
                        }
                        break;
                case DE_CODE:
                        {
                                ...
                                std::string max_file_index_str  =argv[4];

                                uint32_t valid_file_size_arg = stoi(valid_size.data());
                                uint32_t max_file_index_arg  = stoi(max_file_index_str.data());

                                Re_Trans(OUT_FILE_NAME, false,  valid_file_size_arg, max_file_index_arg);
                        }
                        break;
                default:
